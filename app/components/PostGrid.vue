      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="post in posts" :key="post.id"
          ...
          <div class="relative h-64 overflow-hidden">
            ...
            <!-- Date Badge -->
            <div
              class="absolute top-4 left-4 inline-flex px-3 py-1 bg-black/60 backdrop-blur-md rounded-full text-[10px] uppercase tracking-widest font-bold text-white border border-white/10">
          ...
          <div class="p-8">
            <div class="flex items-center gap-3 mb-4">
              <div
                class="w-8 h-8 rounded-full bg-primary-600/20 border border-primary-500/20 flex items-center justify-center text-[10px] font-bold text-primary-400">
            ...
            <div class="flex items-center justify-between pt-6 border-t border-slate-800">
              <NuxtLink :to="'/posts/' + post.id" class="text-xs font-bold text-primary-500 flex items-center gap-2 group/btn">
                READ MORE
                <svg class="w-4 h-4 transform group-hover/btn:translate-x-1 transition-transform" fill="none"
