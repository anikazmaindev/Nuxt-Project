<template>
  <div class="max-w-full mx-auto space-y-10">
    ...
    <!-- Profile Information -->
    <div class="p-8 w-full rounded-3xl bg-slate-900/40 backdrop-blur-xl border border-white/5 space-y-8">
      ...
      <form @submit.prevent="updateProfile" class="space-y-6">
        <div class="space-y-2">
          ...
          <input v-model="profileForm.username" type="text"
            class="w-full px-5 py-3.5 bg-black/20 border border-white/5 rounded-xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all text-white" />
        </div>
        <div class="space-y-2">
        ...
        <p v-if="profileMessage" :class="[profileError ? 'text-red-400' : 'text-green-400']"
          class="text-xs font-bold mt-4">{{ profileMessage }}</p>

        <form @submit.prevent="updatePassword" class="space-y-6">
          <div class="space-y-2">
            <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Current
              Password</label>
            <input v-model="passwordForm.currentPassword" type="password"
              class="w-full px-5 py-3.5 bg-black/20 border border-white/5 rounded-xl focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all text-white" />
          </div>
          <div class="space-y-2">
          ...
          <div class="space-y-2">
            <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Confirm New
              Password</label>
          ...
          <p v-if="passwordMessage" :class="[passwordError ? 'text-red-400' : 'text-green-400']"
            class="text-xs font-bold mt-4">{{ passwordMessage }}</p>
        </form>
