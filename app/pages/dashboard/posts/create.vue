        ...
        <form @submit.prevent="createPost" class="space-y-8">
          <div class="space-y-2">
            <label for="title" class="text-xs font-black uppercase tracking-widest text-slate-500 ml-1">Post
              Title</label>
            <input v-model="title" type="text" id="title" required placeholder="What's the title?"
            ...
            <div class="relative group mt-1">
              <div v-if="imageBase64" class="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
                ...
                <div
                  ...
                  <input id="image-upload-change" type="file" class="sr-only" accept="image/*"
                    @change="handleFileChange" />
                </div>
              </div>

              <label v-else for="image-upload"
                ...
                <span class="text-sm font-bold text-slate-300 group-hover/upload:text-white transition-colors">Drag or
                  click to upload</span>
                <span class="text-[10px] text-slate-600 mt-1 uppercase tracking-widest font-black">PNG, JPG up to
                  2MB</span>
          ...
          <div class="space-y-2">
            <label for="description" class="text-xs font-black uppercase tracking-widest text-slate-500 ml-1">Post
              Description</label>
            <textarea v-model="description" id="description" rows="8" required placeholder="Tell your story..."
              class="w-full px-5 py-4 bg-black/20 border border-white/5 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 outline-none transition-all text-white placeholder:text-slate-700 resize-none font-sans leading-relaxed"></textarea>
          </div>
