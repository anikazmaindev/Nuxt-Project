  ...
  <div class="max-w-2xl mx-auto">
    <div class="bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/5 overflow-hidden">
      <div class="p-8 border-b border-white/5 bg-gradient-to-r from-blue-600/5 to-transparent flex items-center gap-4">
        <NuxtLink to="/dashboard/posts" class="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all border border-white/5">
        ...
        <form @submit.prevent="updatePost" class="space-y-8">
            ...
            <div class="relative group mt-1">
              <div v-if="imageBase64" class="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
                ...
                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <label for="image-upload-edit" class="cursor-pointer px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-lg text-xs font-bold transition-all border border-white/10">
                    Replace Image
                  </label>
                  ...
                  <input id="image-upload-edit" type="file" class="sr-only" accept="image/*" @change="handleFileChange" />
                </div>
              </div>
              
              <label v-else for="image-upload-new" class="flex flex-col items-center justify-center px-8 py-12 border-2 border-dashed border-white/5 rounded-2xl hover:border-blue-500/50 hover:bg-blue-600/[0.02] transition-all cursor-pointer group/upload">
                ...
                <span class="text-sm font-bold text-slate-300 group-hover/upload:text-white transition-colors">Select new image</span>
          ...
          <div class="space-y-2">
            ...
            <textarea
              ...
              rows="8"
              required
              class="w-full px-5 py-4 bg-black/20 border border-white/5 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 outline-none transition-all text-white resize-none font-sans leading-relaxed"
            ></textarea>
          </div>
